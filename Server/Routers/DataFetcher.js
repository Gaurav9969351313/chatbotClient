...
module.exports.checkAppLevelAccess = function (globalCon) {
    // here session created for login is another
    var appLevelAccessFlag = '0';
}

module.exports.getGlobalLevelData = function (globalCon, res) {
    ...
    globalCon.openDoc(config.QlikSense.APP_ID)
        .then((docMetadata) => {
            return docMetadata;
        })
        .then((docMetadata) => {
            ...
            pCreateSessionObject(docMetadata, requestFactory.getRequestStructure("measureListRequest"), "qMeasureList").then((arrQMeasureList) => {
                GlobalData.arrMesureList = arrQMeasureList;
                console.log("arrQMeasureList ", arrQMeasureList);
            })

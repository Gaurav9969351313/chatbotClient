...
export class ConversationComponent implements OnInit, OnChanges {
  ...
  btnClicked(btnName) {
    try {
      ...
      if (this.storageAndUtilsService.getUserData("PrimaryContext") !== null
        && environment.btnsByIndent[a].btns.length > 0) {
  ...
  landingPageLinkClicked(intent) {
    try {
      if (Object.keys(environment.landingPageLinks).includes(intent)) {
        ...
        var urlOrArrOfUrls = this.getSingleConfigratorURL(intent);
        if (typeof urlOrArrOfUrls == "string") {
          this.arrConversation.push(
            {
              type: "iframewithbuttons",
  ...
  getSingleConfigratorURL(intent) {
    try {
      if (typeof environment.btnsByIndent[intent].objId == "string") {
        ...
        var arrObjectIds = environment.btnsByIndent[intent].objId;
        var urlObjects = [];
        var url = "";
        
        for (let i = 0; i < arrObjectIds.length; i++) {
          var objUrl = {};
          if (environment.allowedObjectsWithSelectionBar.includes(arrObjectIds[i].type))
          {
            objUrl["height"] = "250px";
            objUrl["url"] = "https://qsdev.mahindra.com/single/?appid=" + environment.appId + "&obj=" + arrObjectIds[i].id + "&opt=currsel%2cnoanimate&qlikTicket=" + this.qTicket; 
          }
          else 
          {
            objUrl["height"] = "100px";
            objUrl["url"] = "https://qsdev.mahindra.com/single/?appid=" + environment.appId + "&obj=" + arrObjectIds[i].id + "&qlikTicket=" + this.qTicket;
          }
          urlObjects.push(objUrl)
        }
        return urlObjects;
      }
    } catch (error) {
